...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        // [IDENTIFY]
        const ownerNumber = "6283871760857@c.us"
      ...
      if (chats == 'tarik sis'){
	aruga.sendPtt(from, './media/tariksis.mp3', id)
      }
      if (chats == 'Tarik sis'){
	aruga.sendPtt(from, './media/tariksis.mp3', id)
      }   
	...
	aruga.sendPtt(from, './media/tariksis.mp3', id)
      ...
      if (chats == 'iri?'){
        aruga.sendPtt(from, './media/bilangbos.mp3', id)
      }
      if (chats == 'Iri?'){
        aruga.sendPtt(from, './media/bilangbos.mp3', id)
      }
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, '*[LINK GRUP TERDETEKSI]*\nKamu mengirimkan link grup chat, maaf kamu di kick dari grup :(', id).then(() => {
...
// MENU 
        ...
        case 'setpic':
            if (!isOwnerB) return aruga.reply(from, `Perintah hanya dapat digunakan oleh owner bot!`, id)
            if (isMedia) {
            const mediaData = await decryptMedia(message)
            const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
            await aruga.setProfilePic(imageBase64)
            aruga.sendTextWithMentions(`Mengubah foto profile bot berhasil!`)
            } else if (quotedMsg && quotedMsg.type == 'image') {
            const mediaData = await decryptMedia(quotedMsg)
            const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
            await aruga.setProfilePic(imageBase64)
            aruga.sendTextWithMentions(from, `Mengubah foto profile bot berhasil!`)
            } else {
             aruga.reply(from, `Format salah!\nKirim gambar dengan caption ${prefix}setpic`, id)
            }
             break
        ...
        case 'grupmenu':
        case 'groupmenu':
            if (!isGroupMsg) return aruga.reply(from, 'Perintah hanya bisa digunakan didalam grup!', id)
            await aruga.sendText(from, menuId.grupMenu())
            break

        case 'createmenu':
        case 'createdmenu':
            await aruga.sendText(from, menuId.createdMenu())
            break

        case 'islamimenu':
        case 'islammenu':
            await aruga.sendText(from, menuId.islamiMenu())
            break

        case 'searchmenu':
            await aruga.sendText(from, menuId.searchMenu())
            break

        case 'randommenu':
            await aruga.sendText(from, menuId.randomMenu())
            break

        case 'othermenu':
            await aruga.sendText(from, menuId.otherMenu())
            break
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
            ...
            .catch(() => {
                aruga.reply(from, 'Ada yang error, kami akan segera memperbaikinya!', id)
            })
            break
    case 'dare':
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
         ...
         case 'logoff':
            ...
            aruga.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/blackpink?text=jojo`, `${jadiin}.jpg`, 'nehh ngab...', id)
            break
         case 'pornhub':
            if (args.length === 1) return aruga.reply(from, `Kirim perintah *${prefix}pornhub [ /Teks1/Teks2 ]*,\n\n contoh : *${prefix}pornhub /SYAHRUL /BOT*`, id)
            argz = body.trim().split('/')
            if (argz.length >= 2) {
                aruga.reply(from, `Proses!, Mohon ditunggu...`, id)
        ...
        case 'bp':
                ...
                aruga.reply(from, `Proses!, Mohon ditunggu...`, id)
                const namalu = argz[1] 
                aruga.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/blackpink?text=${namalu}`)
            break
        case '3dteks':
        case '3dtext':
                if (args.length === 1)
                argz = body.trim().split('/')
                aruga.reply(from, `Proses!, Mohon ditunggu...`, id)
                const namacoek = argz[1] 
                aruga.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/text3d?text=${namacoek}`)
            break
        case 'thunderteks':
        case 'thundertext':
                if (args.length === 1)
                ...
                aruga.reply(from, `Proses!, Mohon ditunggu...`, id)
                const namalo = argz[1] 
                aruga.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/thunder?text=${namalo}`)
            break
        case 'neontext':
        case 'neonteks':
                if (args.length === 1)
                argz = body.trim().split('/')
                aruga.reply(from, `Proses!, Mohon ditunggu...`, id)
                const namajing = argz[1] 
                aruga.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/neon_light?text=${namajing}`)
            break
        case 'slightning':
        ...
        case 'caklontong':
           ...
           break
        case 'ownerbot':
            await aruga.sendContact(chatId, `6283871760857@c.us`)
            .then(() => aruga.reply(from, 'Tuh nomor ownernya, dichat ya!'))
            break
            case 'maps':
            ...
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                if (isMedia === true) {
                    try {
                        ...
                        var result = await removeBackgroundFromImageBase64({ base64img, apiKey: 'BW7e71zvqgJ6JbRT7d3J8e52', size: 'auto', type: 'auto', outFile })
...
case 'infogempa':
if (!isGroupMsg) return aruga.reply(from, 'Fitur ini hanya bisa digunakan didalam Grup!', id)
const bmkg = await axios.get('https://docs-jojo.herokuapp.com/api/infogempa').then(res => {
        ...
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        aruga.reply(from, 'Ini kak sudah jadi!',id)
                    })
                    .catch(() => {
                        aruga.reply(from, 'Ada yang error!')
                    })
            } else {
                await aruga.reply(from, `Tidak ada gambar! Silahkan kirim gambar dengan caption ${prefix}meme <teks atas> | <teks bawah>\ncontoh: ${prefix}Syahrul SR | BOT `, id)
            }
            break
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, 'Proses ya kak.., mohon ditunggu!', id)
                try {
                    const hasilqmaker = await images.quote(quotes, author, theme)
                    aruga.sendFileFromUrl(from, `${hasilqmaker}`, '', 'Nih udah jadi kak...', id)
                } catch {
                    aruga.reply('Ada yang error, kemungkinan salah penggunaan.', id)
                }
            } else {
                aruga.reply(from, `Pemakaian ${prefix}quotemaker |isi quote|author|theme\n\ncontoh: ${prefix}quotemaker |I love U|Bot Syahrul|random\nUntuk menghindari error, themenya pakai random aja!`)
            }
            break
        case 'nulis':
            if (args.length == 0) return aruga.reply(from, `Membuat bot menulis teks yang dikirim menjadi tulisan dibuku\nPemakaian: ${prefix}nulis <teks>\n\ncontoh: ${prefix}nulis Bot Syahrul SR ganteng banget`, id)
            const nulisq = body.slice(7)
            const nulisp = await rugaapi.tulis(nulisq)
            await aruga.sendImage(from, `${nulisp}`, '', 'Nih udah jadi kak...', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang error, kemungkinan salah penggunaan.', id)
            })
            break
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us','')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
        ...
        case 'bc':
            ...
            for (let ids of chatz) {
                var cekgrup = await aruga.getChatById(ids)
                if (!cekgrup.isReadOnly) await aruga.sendText(ids, `[ SYAHRUL SR BROADCAST ]\n\n${msg}`)
            }
